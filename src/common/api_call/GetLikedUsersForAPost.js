...
async function getLikedUsersForAPost(postShortCode, isSleeping, stopAPICallCheck, nextPageToken = null, previousUsers = []) {
  if (stopAPICallCheck()) {
  ...
  try {
    ...
    isSleeping(true, new SleepArgs(false, "LIKED_USERS_OF_POST_PRIVATE"));
    await sleep(options.sleepTimeFor429InSeconds * 1000);
    return await getLikedUsersForAPost(postShortCode, isSleeping, stopAPICallCheck, nextPageToken, previousUsers);
  }
  ...
  if (pageInfo.has_next_page) {
    return await getLikedUsersForAPost(postShortCode, isSleeping, stopAPICallCheck, pageInfo.end_cursor, previousUsers);
  } else {
    return previousUsers;
  }
}
...
export default getLikedUsersForAPost;
